...
VaIMod.plugin({
  ...
  //   waitVm  : 等 vm 就绪再执行 code；期间标签页显示「插件加载中…」而不是空面板。
  //             等不到（超时/桥接出错）视为**加载失败**，不执行 code ——
  //             避免「只该在有 vm 时工作」的插件带着空数据产生副作用。
  //   load    : 预加载钩子，在 code 之前执行，可 async；用于按需拉远程资源 /
  //             初始化大依赖。抛错同样按失败处理（code 可能依赖它）。
  //   timeout : 上两项的等待上限（ms）；0 或不写 = 不限制
  //             （面板侧仍有 120s 兜底上限，防止永不终止的等待挂住定时器）。
